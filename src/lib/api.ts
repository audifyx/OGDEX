...
export const getScreener = (type: string, interval: string, limit = 100, chain = "solana") =>
 j<{ rows: Row[]; count?: number; error?: string; chain?: string }>(`/api/screener?type=${type}&interval=${interval}&limit=${limit}&chain=${chain}`);
...
export const getBoosts = () => j<{ ok: boolean; boosts: Boost[]; error?: string }>("/api/boosts");
export const getBoostTiers = () => j<{ tiers: any[]; payWallet: string }>("/api/boosts?tiers=1");
export const submitBoost = (data: any) => postJson("/api/boosts", data) as Promise<{ ok: boolean; boost?: Boost; error?: string }>;
export const search = (q: string) => j<{ rows: Row[] }>(`/api/search?q=${encodeURIComponent(q)}`);
export const getToken = (mint: string) => j(`/api/token?mint=${mint}`);
...
export const getChart = (mint: string, interval = "1h", limit = 200, chain = "solana") =>
 j(`/api/chart?mint=${mint}&interval=${interval}&limit=${limit}&chain=${chain}`);
...
export const getWallet = (address: string) => j(`/api/wallet?address=${address}`);
...
export const adminGet = (pass: string) => j(`/api/admin?pass=${encodeURIComponent(pass)}`);
